...
for machine in input:
    ...
    lightparity = cp.Variable(len(lights), integer = True)
    buttons_var = cp.Variable(len(buttons), boolean = True)
    #lights should be either even or odd
    constraints = []
    #add light constraints
    for i in range(len(lights)):
        constraints.append(lights_var[i]>=0)
        if lights[i]:
            constraints.append(lights_var[i] == 2 * lightparity[i] + 1)
        else:
            constraints.append(lights_var[i] == 2 * lightparity[i])
        toggles = 0
